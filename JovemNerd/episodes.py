...
class Collector:
    ...
    def save_parquet(self, data):
        ...
        save_path = f"./data/episodes/parquet/{self.instance_name}/parquet/"
        os.makedirs(save_path, exist_ok=True)  # Cria o diretório, se necessário
        df = pd.DataFrame(data)
        df.to_parquet(f"{save_path}/{now}.parquet", index=False)


    def save_json(self, data):
        now = datetime.datetime.now().strftime("%Y%m%d_%H%M%S.%f")
        save_path = f"./data/episodes/json/{self.instance_name}/json/"
        os.makedirs(save_path, exist_ok=True)  # Cria o diretório, se necessário
