...
def save_data(data,option='json'):
    ...
    if option == 'json':
        with open(f"/Data - Collect/TabNews/data/contents/json/{now}.json", 'w') as open_file:
            json.dump(data, open_file, indent=4)

    elif option == 'parquet':
        df = pd.DataFrame(data)
        df.to_parquet(f"/Data - Collect/TabNews/data/contents/json/{now}.parquet", index=False)
